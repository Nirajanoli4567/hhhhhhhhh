...
import Sidebar from "./PharSidebar";

function Medicinelist() {
  // Sample patient data with billing status
  const [patients, setPatients] = useState([
    { id: 1, name: "John Doe", age: 30, gender: "Male", billStatus: "Pending" },
    { id: 2, name: "Jane Smith", age: 25, gender: "Female", billStatus: "Paid" },
    { id: 3, name: "Emily Johnson", age: 40, gender: "Female", billStatus: "Pending" },
    { id: 4, name: "Michael Brown", age: 35, gender: "Male", billStatus: "Paid" }
  ]);
          ...
          {filteredPatients.length === 0 ? (
            ...
            <ul className="space-y-4">
              {filteredPatients.map(patient => (
                <li key={patient.id} className="flex justify-between items-center p-4 border border-gray-300 rounded-md">
                  <div>
                    <p className="font-semibold">{patient.name}</p>
                    <p>Age: {patient.age}</p>
                    <p>Gender: {patient.gender}</p>
                    <p>Status: <span className={patient.billStatus === "Paid" ? "text-green-600" : "text-red-600"}>{patient.billStatus}</span></p>
                  </div>
